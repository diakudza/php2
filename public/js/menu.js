...
function ctlMenuClose(){
 ...
 let cartRemoveButtons = document.querySelectorAll('.cart__card-close');

 productButtons.forEach(button => button.addEventListener('click', () => {count.innerText = ++itCount;}));
 cartRemoveButtons.forEach(button => button.addEventListener('click', () => {count.innerText = --itCount;}));
...
function clearCart(userId) {
    let idCount = document.getElementById('count')
       $.ajax({
        ...
        success: function(data){
            console.log(data)
            idCount.classList.toggle('visually-hidden')
            $(".card__column").html('');
